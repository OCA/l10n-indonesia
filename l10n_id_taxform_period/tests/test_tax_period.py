...
class TaxPeriodCase(TransactionCase):
    ...
    def test_create_year(self):
        ...
        tax_year = self.obj_year.create(
            {
                "name": str(next_year),
                "code": str(next_year),
                "date_start": date_start_year.strftime("%Y-%m-%d"),
                "date_end": date_end_year.strftime("%Y-%m-%d"),
            }
        )
        ...
        self.assertEqual(
            self.obj_year._find_year(date_start_year.strftime("%Y-%m-%d")), tax_year
        )
        # Check l1on_id.tax_period _find method
        self.assertEqual(
            self.obj_period._find_period(date_start_year.strftime("%Y-%m-%d")),
            tax_year.period_ids[0],
        )
        # Check forward find l1on_id.tax_period _find method
        self.assertEqual(tax_year.period_ids[1]._next_period(1), tax_year.period_ids[2])
        # Check backward find l1on_id.tax_period _find method
        self.assertEqual(
            tax_year.period_ids[2]._previous_period(1), tax_year.period_ids[1]
        )
