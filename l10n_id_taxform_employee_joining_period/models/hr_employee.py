...
class HrEmployee(models.Model):
    ...
    def _compute_tax_period(self):
        for employee in self:
            ...
            employee.quiting_tax_period_id = False
            employee.quiting_tax_year_id = False
            ...
            contract = obj_contract.search(
                criteria, order="date_start desc", limit=1)
            if not contract:
                continue
            ...
            try:
                ...
                period = obj_period._find_period(
                    contract.date_end)
                employee.quiting_tax_period_id = period
                employee.quiting_tax_year_id = period.year_id
            except:
                continue
    ...
    quiting_tax_period_id = fields.Many2one(
        string="Quiting Tax Period",
        comodel_name="l10n_id.tax_period",
        compute="_compute_tax_period",
        store=False,
        )
    quiting_tax_year_id = fields.Many2one(
        string="Quiting Tax Year",
        comodel_name="l10n_id.tax_year",
        compute="_compute_tax_period",
        store=False,
        )
